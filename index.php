<!DOCTYPE html>
<html>
	<head>
		<title>about - Mirco Kocher</title>
		<link rel="stylesheet" type="text/css" href="style.css">
	</head>
	<body>
		<div class="navbar">
			<ul>
				<li><a class="navbar-home" href="http://mirc0crim.github.io">Mirco</a></li>
				<li><a class="navbar-now" href="http://mirc0crim.github.io">Home</a></li>
				<li><a class="navbar-nav" href="http://mirc0crim.github.io/publications.html">Publications</a></li>
			</ul>
		</div>
		<div class="container">
			<div class="main">
				<h1>about</h1>
				<p>
					I am a second year PhD student in the <a href="http://www.unine.ch/iiun/page-5110_en.html">Computational Linguistics group</a> at the <a href="http://www2.unine.ch/cms/lang/en/pid/5100_en">Computer science department</a>,<a href="http://www.unine.ch/">Universit&#233; de Neuch&#226;tel</a> supervised by <a href="http://members.unine.ch/jacques.savoy/">Prof. Jacques Savoy</a>. My research focus is on the design, implementation, and evaluation of text clustering algorithms based on stylistic features. The project "Clustering with Styles" is part of the <a href="http://style.unine.ch/">Computational Lingusitics and Stylometry Studies</a>.
				</p>
				<p>
					I received my Bachelor of Science in Computer Science (<i>cum laude</i>) from the <a href="http://www.unibe.ch/eng/">University of Bern</a>, Switzerland. I got the <a href="http://mcs.unibnf.ch/">Swiss Joint Master of Science in Computer Science</a> (<i>summa cum laude</i>) of the Universities of <a href="http://www.unibe.ch/eng/">Bern</a>, <a href="http://www.unine.ch/">Neuch&#226;tel</a> and <a href="http://www.unifr.ch/home/welcomeE.php">Fribourg</a>.
				</p>
			</div>
			<div class="me">
				<p>
					Mirco Kocher
					<br/>
					<a href="http://www2.unine.ch/cms/lang/en/pid/5100_en">Computer science department</a>
					<br/>
					<a href="http://www.unine.ch/">Universit&#233; de Neuch&#226;tel</a>
				</p>
				<p>
					Room B123
					<br/>
					Rue Emile-Argand 11
					<br/>
					2000 Neuch&#226;tel, Switzerland
					<br/>
					e-mail: firstname.lastname/AT/unine.ch
				</p>
				<p>
					<a href="http://dblp.uni-trier.de/pers/hd/k/Kocher:Mirco">My dblp</a>
				</p>
			</div>
		</div>
		<?php include ("footer.php"); ?>
	</body>
</html>

